...
//        request.region = this.options.country;
    ...
    myhandleGeocode: function(results, status){
        if (status === google.maps.GeocoderStatus.OK) {
	        var result = results[0];
	        this.fillDetails(result);
	        this.trigger("geocode:myresult", result);
        } 
      },
    mygeocode: function(address,func){
    	var request = {
          address: address || this.$input.val()
        };
        if (this.options.bounds && !request.bounds){
...
//          request.region = this.options.country;
        ...
        {
        	var callback = this.myhandleGeocode;
        }
        this.geocoder.geocode(request, $.proxy(callback, this));
      },
