...
class JConfig
{
    ...
    public static function item($group)
    {
        ...
        {
        	$conf_file = Yii::getPathOfAlias('application.config') . '/'.$group.'.php';
        	if(!file_exists($conf_file))
        	{
        		return "";
        	}
            $config[$group] = new CConfiguration($conf_file);
        }
    ...
    public static function is_array($value)
    {
   	...
   	{
   		$fee = !self::item("common.fee")?8:self::item("common.fee");
   		$first_discount = self::item("common.first_discount");
   		if($first_discount!="")
   		{
   			if(!Yii::app()->user->isGuest)
   			{
   				$have = (int) Order::model()->countByAttributes(array('uid'=>Yii::app()->user->id));
   				if($have === 0)
   				{
   					$fee = $fee*$first_discount/100;
   				}
   			}
   		}
   		return $fee;
